-- from cs/google3/experimental/users/fentanes/nvgoog/lua/nvgoog/util.lua
...
local autocmd_group = vim.api.nvim_create_augroup("verylazy", { clear = false })
...
-- Only supports `*`, no other glob patterns such as `?`.
...
local function loadPlugin(spec)
  ...
  local plugin_name = spec.name or spec.dir and get_name(spec.dir) or spec.url and get_name(spec.url)
            ...
            vim.api.nvim_buf_call(bufnr, function()
              for _, autocmd in ipairs(plugin_autocmds) do
                if not autocmd.buflocal or autocmd.buffer == bufnr then
                  local match = autocmd.event == "FileType" and vim.bo.filetype or vim.api.nvim_buf_get_name(bufnr)
